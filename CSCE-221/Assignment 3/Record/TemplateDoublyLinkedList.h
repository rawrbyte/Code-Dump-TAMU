...
public:
  ...
  DListNode<T>* insertOrderly(const T& obj); //inserts an object to the correct position assuming the linked list is sorted
};
...
DListNode<T>* DoublyLinkedList<T>::insertOrderly(const T& obj){
    DListNode<T>* tmp = header.getNext();
    while(tmp != this->getAfterLast()){
        if(obj < tmp->getElem()){
            DListNode<T>* node= new DListNode<T>(obj, tmp->getPrev(), tmp);
            tmp->prev->next= node;
            tmp->prev = node;
            return node;
        }
    ...
    DListNode<T> *node = new DListNode<T>(obj, trailer.prev,&trailer);
